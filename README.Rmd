...
nano.counts <- read.csv("man/figures/nano.package.csv")
row.names(nano.counts) <- nano.counts$STRING_gene
...
```{r identify degs,message=FALSE, warning=FALSE}
...
Another available method for calculating the co-expression values is using the WGCNA package correlation powered analysis. The function genes_coexpression() offers an easy way of using this method.
```{r coexps, eval=FALSE, message=FALSE, warning=FALSE}
